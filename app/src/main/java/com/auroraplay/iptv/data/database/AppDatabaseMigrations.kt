...
object AppDatabaseMigrations {
    ...
    // Optional mirror server per connection, tried automatically when the
    // primary is unreachable. Additive, nullable — existing rows are simply
    // "no backup configured" until the user sets one.
    private val MIGRATION_12_13 = object : Migration(12, 13) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `connections` ADD COLUMN `backupServerUrl` TEXT")
        }
    }
