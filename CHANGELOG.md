...
- **Canais: categorias refeitas por gênero.** Os canais do provedor são
  agrupados automaticamente em Abertos, Esportes, Notícias, Filmes,
  Séries, Infantil, Documentários, Música, Novelas, Religioso e
  Internacionais. Categorias sem canal não aparecem. Ao abrir um canal,
  os filtros pulam para a categoria dele.
- **Busca: a aba "Canais" saiu** — a busca agora é de filmes e séries, e
  os três filtros (Tudo/Filmes/Séries) ocupam a linha toda. Buscar um
  gênero ("anime", "terror", "romance"…) traz o resultado em trilhos por
  categoria; buscar um título continua em lista simples.
- Site do projeto refeito: mais simples, mais direto e sem sumir em
  alguns navegadores.

## 1.39.0 — 2026-09-07

- **Só retrato no celular.** O app trava em retrato em celular e tablet
  (as telas foram feitas pra ele); só o player em tela cheia gira. Android
  TV não muda.
- **Ocultar conteúdo adulto** (Ajustes → Conteúdo, ligado por padrão):
  esconde canais e filmes de categorias +18 em todos os perfis. Vale na
  hora, sem fechar o app.
- **Canal ao vivo não trava mais** ao fechar a tela cheia e voltar para a
  prévia.
- **"AO VIVO" virou botão** (topo do player): toque para pular direto para
  o ao vivo real. Botão **"Miniatura"** para entrar em Picture-in-Picture
  na hora.
- **Lista de canais no player** aparece ao lado do vídeo (sem escurecer),
  abre na categoria do canal que está tocando e rola até ele; favoritar
  direto na lista.
- **Busca mais rápida** ao abrir o app; **modo cinema** agora só em filmes
  e séries (menos peso no ao vivo).
- Site do projeto refeito: mais simples e direto.

## 1.38.5 — 2026-09-07

- **Menu de canais no player** refeito: filtros por categoria e "Favoritos",
  busca que ignora acentos, contagem de resultados e um estado vazio de
  verdade. Abre já na categoria do canal que está tocando e rola até ele;
  favoritar/desfavoritar direto na lista.
- O menu de canais **não abre mais o teclado sozinho** — a busca é acionada
  ao tocar no campo.
- Os três painéis laterais do player (canais, episódios, guia) passaram a
  compartilhar o mesmo layout, com animação de entrada e respeito às áreas
  do sistema.
- Site do projeto atualizado (listas M3U, XMLTV, controle remoto, servidor
  reserva) e histórico de versões revisado.
